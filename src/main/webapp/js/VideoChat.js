class VideoChat {
	constructor(ko) {
		...
		this.ws.onmessage = function(event) {
			...
			if (data.type=="OFFER") {
				self.anunciarLlamada(data.remitente, data.sessionDescription, data.recipient);
				return;
			}
			...
			if (data.type=="ANSWER") {
				...
				if (sessionDescription == null) {
					window.confirm("Llamada de " + data.recipient + " rechazada");
				}
	...
	anunciarLlamada(remitente, sessionDescription, recipient) {
		...
		if (aceptar)
			...
			this.rechazarLlamada(remitente, sessionDescription, recipient);			
	}
	
	aceptarLlamada(remitente, sessionDescription) {
		if (!this.videoLocalOn)
			this.encenderVideoLocalAceptandoVideollamada(remitente, sessionDescription);
	}
	...
	encenderVideoLocal(destinatario) {
		...
		navigator.getUserMedia(
			...
			function(stream) {
				...
				if (!self.conexion){
					self.crearConexion(destinatario);
				}
			}, 
			function(error) {
	...
	encenderVideoLocalAceptandoVideollamada (remitente, sessionDescription) {
		let self = this;
		...
		navigator.getUserMedia(
			...
			function(stream) {
				...
				self.addMensaje("Vídeo local conectado", "green");
				if (!self.conexion){
					self.crearConexionAceptandoVideollamada(remitente, sessionDescription);
				}
			}, 
			function(error) {
				self.addMensaje("Error al cargar vídeo local: " + error, "red");
	...
	crearConexionAceptandoVideollamada(remitente, sessionDescription) {
		let self = this;
		let servers = { 
			iceServers : [ 
	...
	enviarOferta(destinatario) {
		let self = this;
		if (!this.videoLocalOn){
			this.encenderVideoLocal(destinatario);
		}
		//if (!self.conexion){
			//self.crearConexion();
	//	}
